...
import { listDrafts, getDraft, updateDraft, sendDraft, deleteDraft } from "./draft-manager.js";
...
}).describe("Replaces a draft's content with new email data");

const SendDraftSchema = z.object({
  draftId: z.string().describe("ID of the draft to send"),
}).describe("Sends an existing draft email");

const DeleteDraftSchema = z.object({
  draftId: z.string().describe("ID of the draft to permanently delete"),
}).describe("Permanently deletes a draft");
...
async function main() {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      {
        name: "list_drafts",
        description: "Lists drafts in the user's mailbox with subject, recipients, and date",
        inputSchema: zodToJsonSchema(ListDraftsSchema),
      },
      {
        name: "get_draft",
        description: "Gets the full content of a specific draft by ID",
        inputSchema: zodToJsonSchema(GetDraftSchema),
      },
      {
        name: "update_draft",
        description: "Replaces a draft's content (recipients, subject, body, attachments)",
        inputSchema: zodToJsonSchema(UpdateDraftSchema),
      },
      {
        name: "send_draft",
        description: "Sends an existing draft to its recipients",
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    async function handleEmailAction(action: string, validatedArgs: any) {
      let message;
      try {
        if (validatedArgs.attachments && validatedArgs.attachments.length > 0) {
          ...
          if (action === "send") {
            ...
            return {
              content: [{ type: "text", text: `Email draft created successfully with ID: ${response.data.id}` }],
            };
          }
        } else {
    ...
    async function processBatches(items: any[], batchSize: number, processFn: (batch: any[]) => Promise<any[]>) {
      const successes: any[] = [];
      const failures: any[] = [];
    ...
    try {
      switch (name) {
        ...
        case "read_email": {
          ...
          const processAttachmentParts = (part: any) => {
            ...
            if (part.parts) {
              part.parts.forEach((subpart: any) => processAttachmentParts(subpart));
            }
          };
        ...
        case "modify_email": {
          ...
          return {
            content: [{ type: "text", text: `Email ${validatedArgs.messageId} labels updated successfully` }],
          };
        }
        ...
        case "delete_email": {
          ...
          return {
            content: [{ type: "text", text: `Email ${validatedArgs.messageId} deleted successfully` }],
          };
        }
          ...
          return {
            content: [{
              ...
              text: `Found ${labelResults.count.total} labels (${labelResults.count.system} system, ${labelResults.count.user} user):\n\n` +
                "System Labels:\n" +
                labelResults.system.map((l: any) => `ID: ${l.id}\nName: ${l.name}\n`).join('\n') +
                "\nUser Labels:\n" +
                labelResults.user.map((l: any) => `ID: ${l.id}\nName: ${l.name}\n`).join('\n')
            }],
          };
        }
        ...
        case "batch_modify_emails": {
          ...
          if (failures.length > 0) {
            ...
            resultText += failures.map((f: any) => `- ${f.item.substring(0, 16)}... (${f.error.message})`).join('\n');
          }
        ...
        case "list_filters": {
          ...
          if (result.filters.length === 0) {
            return { content: [{ type: "text", text: `No filters found. Debug raw response: ${JSON.stringify(result._debug_raw)}` }] };
          }
        ...
        case "get_filter": {
          ...
          const actionText = Object.entries(result.action || {})
            ...
            .join(', ');
          return {
            content: [{ type: "text", text: `Filter details:\nID: ${result.id}\nCriteria: ${criteriaText}\nActions: ${actionText}` }],
          };
        }
        ...
        case "create_filter_from_template": {
          ...
          switch (validatedArgs.template) {
            ...
            case 'mailingList':
              ...
              break;
            default:
              throw new Error(`Unknown template: ${validatedArgs.template}`);
          }
          const result = await createFilter(gmail, filterConfig.criteria, filterConfig.action);
          return {
            content: [{ type: "text", text: `Filter created from template '${validatedArgs.template}':\nID: ${result.id}\nTemplate used: ${validatedArgs.template}` }],
          };
        }
        ...
        case "list_drafts": {
          ...
          const results = await listDrafts(gmail, validatedArgs.maxResults);

          if (results.length === 0) {
            return { content: [{ type: "text", text: "No drafts found." }] };
          }

          const text = results.map((d: any) =>
            `ID: ${d.id}\nSubject: ${d.subject}\nTo: ${d.to}\nDate: ${d.date}\nThread: ${d.threadId || 'none'}\n`
          ).join('\n');

          return {
            content: [{ type: "text", text: `Found ${results.length} drafts:\n\n${text}` }],
          };
        }
        ...
        case "get_draft": {
          ...
          const { text, html } = extractEmailContent(draft.message?.payload || {});
          const body = text || html || '';
          const contentTypeNote = !text && html
            ? '[Note: This draft is HTML-formatted. Plain text version not available.]\n\n'
            : '';

          return {
            content: [{
              type: "text",
              text: `Draft ID: ${draft.id}\nMessage ID: ${draft.message?.id}\nThread ID: ${draft.message?.threadId || 'none'}\nSubject: ${subject}\nFrom: ${from}\nTo: ${to}${cc ? `\nCc: ${cc}` : ''}\nDate: ${date}\n\n${contentTypeNote}${body}`,
            }],
          };
        }
        ...
        case "update_draft": {
          ...
          const { draftId, ...emailArgs } = validatedArgs;
          const result = await updateDraft(gmail, draftId, emailArgs);
        ...
        case "send_draft": {
          ...
          const result = await sendDraft(gmail, validatedArgs.draftId);

          return {
            content: [{ type: "text", text: `Draft sent successfully.\nMessage ID: ${result.id}\nThread ID: ${result.threadId}` }],
          };
        }

        case "delete_draft": {
          const validatedArgs = DeleteDraftSchema.parse(args);
          const result = await deleteDraft(gmail, validatedArgs.draftId);
